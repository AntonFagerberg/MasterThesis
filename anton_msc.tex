...
\subsection{Pipeline}
Pipelines is what ties all the pieces in Rackla together. The goal, when writing an API gateway using Rackla, is to describe the process as a sequence of steps. The simplest step is to proxy a request in the API gateway---that is to relay the request without modifying it. We can do that by first defining a URL to call, pipe it to the request function and then pipe it to the response function:
...
|> response
\end{lstlisting}
  \caption{A simple proxy which relay the entire payload untouched.}
\end{figure}

\subsection{Monads and Functional programming}
In order for the pipeline to work, a new type has to be defined. This type has to be able to handle multiple requests asynchronously while still enable us to modify the underlying values before they are sent to the clients.
...
The new \lstinline{Rackla} type defines what it means to chain operations together using well established concepts used in functional programming such as \lstinline{map}, \lstinline{flat\_map} and \lstinline{reduce} with focus on still being asynchronous.
...
It is worth pointing out here that the new \lstinline{Rackla} type allows this pipeline to be fully asynchronous, meaning that the response order sent to the client will depend on which of image urls is responding first and which of the Base64 encodings is performed the fastest. It can very well be the case that one Base64 encoded image has already been fully sent to the client while the other image is still being requested in the API gateway.

\subsection{Function overview}
We will now explain the most common functions defined in Rackla which enables us to build very flexible and powerful API gateways.

\subsubsection{Request}
The function \lstinline{request}, in the simple case, takes one or many URLs as strings and returns a \lstinline{Rackla} type. The data eventually contained inside the \lstinline{Rackla} type will correspond to either the result of the HTTP request or an error tuple if any error occur such as DNS lookup failures or non-responding back-ends.

More advanced HTTP requests can be executed by instead passing one or many key-value maps to the \lstinline{request} function. By using a map, we can define which HTTP verb to use such as GET or POST, what timeout limits we want to use and how we want to handle insecure SSL connections. We can also, by using a key-value map, define custom request headers and attach a request body (payload).

We can also define here if we want the \lstinline{Rackla} type to contain just the response body or a map containing, in addition to the response body, the response status code and response headers.

Optionally, we can tell \lstinline{request} if we are interested in the entire HTTP response including the HTTP status code and the headers or if we, as is default, are only interesting in the body (payload).
...
The function \lstinline{response} takes a \lstinline{Rackla} type and converts the underlying types to an HTTP response and sends it to the client. In order to conform to the asynchronous nature of Rackla, we only use chunked responses\cite{chunked}. We can pass in options in order to customise the response as well. Available options are: \lstinline{json} which automatically encodes the response as a JSON data structure, \lstinline{compress} which compresses the response using GZip compression, \lstinline{sync} which guarantees that the responses are sent in the same order as were requests were made, \lstinline{status} sets the response status code and \lstinline{headers} which defines the response headers used in the HTTP response.

\subsubsection{Map}
The function \lstinline{map} is used in order to transform the content inside the \lstinline{Rackla} type from one type to another type. As an example, we saw in Figure \ref{fig:map} that we can apply a Base64 encoding to each individual image's data from many HTTP requests contained inside a \lstinline{Rackla} type.

\subsubsection{Flat map}
The function \lstinline{flat_map} works like \lstinline{map} with the exception that the returned type inside it has to be a new \lstinline{Rackla} type. The naive approach is to just wrap the returned type using \lstinline{just}, but more appropriate is to use this function when you want to utilise the \lstinline{response} function inside.
...
|> response
\end{lstlisting}
  \caption{A new pipeline can be built inside the flat\_map function.}
\end{figure}

In the above figure we make an HTTP request to an end-point which will return a response. By then using \lstinline{flat_map}, we can create one or more additional requests from the response by creating a new pipeline inside the \lstinline{flat_map} function. This enables us to define recursive request structures which later boils down to a single response.

\subsubsection{Reduce}
The function \lstinline{reduce}, or sometimes called \lstinline{fold}, is a way to take a collection of values and \enquote{reduce} them into to a single value. Rackla provides a \lstinline{reduce} function which reduces the values contained inside its own type into a single value. An example of this is when several responses from HTTP requests are reduced into a single value of some type.
...
Sometimes we may want to to break out of the asynchronous \lstinline{Rackla} type. In these cases, we can use the function \lstinline{collect} which takes a \lstinline{Rackla} type and returns the containing value or values as a list if there are more than underlying value.
...
To conclude this section, we will look at a complete end-point which uses the Instagrams API. We will present the code below and then explain what happens line by line. This is a complex end-point which uses most of the techniques defined in Rackla.
...
\item Line 1: we define our new HTTP end-point which we name \lstinline{/instagram}.

\item Line 2--4: we create our first pipeline. What we want to do is to first send some HTML tags which will make the response a valid HTML conforming response. Normally we would not do this since we are working with API responses---but in this case to illustrate an example, we will create a complete HTML response page. We do this by storing the HTML code in a string, use the function \lstinline{just} to turn it in to a \lstinline{Rackla} type and pipe it to the \lstinline{response} function. Since the pipeline is ended with a \lstinline{response} function, it is immediately sent to the client before moving on.

\item Line 6: we define the underlying back-end URL to the end-point we wish to use. The end-point requires an access token which we pass to it by letting the client supply it as the query string.

\item Line 7: we pipe our URL to the \lstinline{request} function which will immediately return a \lstinline{Rackla} type while in the background execute the HTTP request.

\item Line 8--34: we use \lstinline{flat_map} with an anonymous lambda function.

\item Line 9: we pattern match on the response data from the http request. We know that the response will either be the actual response body from the API endpoint or an error tuple if case an error has occured. 

\item Line 10--11: if we receive an error, we will transmit the reason the error occured to the client. We have to call the \lstinline{just} function to convert it into a \lstinline{Rackla} type since it is required from the definition of\lstinline{flat_map}.

\item Line 13: if we have reached this point in the code, then we know that we have gotten a valid HTTP response.

\item Line 14: we pass the HTTP response data to the function \lstinline{Poison.decode} which will decode the response data from a JSON string to an Elixir data structure with an external library\cite{poison}. This conversion can fail as well so we will yet again pattern match on the outcomes of this operation.

\item Line 15--16: we handle failing JSON decoding by just transmitting the failure reason to the client.
...
\item Line 19--21: we transform the complete JSON response by extracting just the image URLs. This will leave us with a list of URLs pointing to images. It is worth pointing out that we are mixing types within the different stages of the pipeline and that we are actually working with both the built in Elixir types and the \lstinline{Rackla} type seamlessly.

\item Line 22: we pipe the list of URLs pointing to images to the \lstinline{request} function. It is important to notice what is happening here: we transform the response, from the outside pipeline's request, to be a new request pipeline which contain many requests. Also note that we are not providing any \lstinline{response} function here, we will instead use the \lstinline{response} function from the outer pipeline.

\item Line 23: we map over the results from our internal pipeline.

\item Line 24: we pattern match again on the data. Since we have executed HTTP requests once again we may end up with failing DNS lookups and other problems which we have to handle.
...
\item Line 29: we Base64 encode the image data and add wrap it in a HTML image tag which enables it to render directly on our page.

\item Line 35: we end up in the \lstinline{response} function defined in the outer pipeline. This function will asynchronously receive the image HTML tags from our internal pipeline and transmit it to the client. It is important to underline the asynchronous behaviour we have defined here---the images will be sent in nondeterministic order which depends on the response order of the image requests.

\item Line 37--39: we define our final pipeline which will send the closing HTML tags to the client. We are at this point guaranteed that these tags will be sent after every image has been transmitted since this final pipeline is defined separately from the previous pipelines---the asynchronous behaviour is only applicable inside the same pipeline.

\end{itemize}

When we visit our new endpoint \lstinline{/instagram}, we will see a valid HTML page which will contain images fetched from Instagram. Since we use chunked transfer encoding, we will see every image as soon as it is sent from our API gateway. Since we are not using a deterministic ordering and simply send each image as soon as it is available, we will get a different ordering of the images every time we refresh the page.
...
To conclude we should point out that we would perhaps in a real world application rather request the image data as Base64 encoded string in chunks and apply the HTML markup inside of the client's JavaScript code. By doing so we would not have to mix in any HTML in our API gateway. By specifying the image order in the chunks, the client could also chose render the images in the position in a grid even though the images arrives in a different order.
