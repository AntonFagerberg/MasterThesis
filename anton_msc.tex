  ...
  \centering
    \begin{center}
      \begin{tabular}{ r | c | c }
        ...
        Total headers & 12,410 B & 1,241 B \\ \hline
        Total payload & 2,710 B & 2,710 B \\ \hline
        \% headers of total data & 82\% & 31\% \\ \hline
      \end{tabular}
    \end{center}
  \caption{The headers-payload ratio when fetching ten user in one request compared to one request per user. The overhead percentage will continue to decrease linearly in relation to the number of users requested simultaneously.}
\end{figure}
...
Another interesting thing in Figure~\ref{fig:headers_overhead} happens on line 46--50 which is where the payload is. While it looks like gibberish, it is actually the user profile data in JSON format seen in Figure~\ref{fig:user_json} but compressed using Gzip (more about compression on page~\pageref{compression}). Compressing responses is a great bandwidth saving technique---however, only the payload can be compressed in HTTP/1.1, not the headers.
