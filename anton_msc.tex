...
\keywords{API, gateway, proxy, communication, performance, HTTP, Elixir}
...
This new software layer between the client and server corresponded to the concept of an API-gateway. This thesis consists of four parts which builds upon each other. 

First we look at the transport protocol HTTP, especially HTTP/1.1, and what problems appear when the server and client does not communicate efficiently.

Secondly, we theorise around the broad concept of API-gateways. Here we try to define different ways how the API-gateway can improve the relationship between clients and servers, and how the problems defined in the previous section can be solve.

Thirdly, an API-gateway framework was written to gain a better understanding of the problem from a practical and technical point of view. This framework allowed us to deeper understand and benchmark the problems defined in earlier chapters and provide real solutions.

Finally, we did a case study on real-world production systems. This consisted of an analysis to determine whether the systems had issues which could be resolved with an API-gateway. A solution was then created for that system in the previously written framework. This was done in order to verify that the framework could be used in real-world scenarios but also to be able to benchmark the results both before and after the introduction of it.
