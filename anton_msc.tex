...
Note that this is just one approach to some of the problems --- there subtler problems other such as the fact that you can have comments in XML but not in JSON. As there is no 1:1 mapping for XML and JSON, different library developers and corporations are developing their own methods to do this conversion. 

A notorious example is JSONx which is IBMs standard format for representing JSON as XML\cite{jsonx}. This format went viral as a bloated enterprise format when a person developed a tool which converted Json to XML with IBMs JsonX and then back to JSON from XML with JsonML\cite{jsonml}. Since these tools all follow their own defined conversion standard, the mapping can not be 1:1 and because of this the output from this recursion will grow with the increasingly added verbose meta-data until the browser crashes.

\section{API over XML}
In XMLThing, the XML-JSON conversion was handled in two different ways. When requests are executed to the API, the client has a built in service which takes all responses and converted the XML-structure to JSON before passing the data down to the views or controllers. In this way, the client 
