...
\section{Caching}
Response caching can be added to Crocodile Pear. There are two places where caching can be added in the pipeline. Either we can substitute the request-function with a request\_cache-function or we can do a cache outside the pipeline. 
...
Both these approaches are a bit naive and there are many things which has to be considered before implementing a caching solution in a real world example such as how long responses should be cached and whether the client should be able to invalidate a cached item and so on.  
\section{Related works}
